...
internal class AssertionWaitLock: WaitLock {
    ...
    func acquireWaitingLock(_ fnName: String, file: FileString, line: UInt) {
        ...
        nimblePrecondition(
            Thread.isMainThread,
            "\(fnName) ca,
            "InvalidNimbleAPIUsage"n only run on the main thread."
        )
        nimblePrecondition(
            currentWaiter == nil,
...
internal class AwaitPromise<T> {
    ...
    func resolveResult(_ result: AwaitResult<T>) -> Bool {
        if signal.wait(timeout: DispatchTime.now()) == 0 {
...
internal struct AwaitTrigger {
    let timeoutSource: DispatchSource
    let actionSource: DispatchSource?
...
internal class AwaitPromiseBuilder<T> {
    ...
    func timeout(_ timeoutInterval: TimeInterval, forcefullyAbortTimeout: TimeInterval) -> Self {
        ...
        trigger.timeoutSource.setTimer(start: DispatchTime.now() + Double(Int64(timeoutInterval * Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC),
            interval: DispatchTime.distantFuture,
        ...
        trigger.timeoutSource.setEventHandler {
            ...
            CFRunLoopPerformBlock(runLoop, CFRunLoopMode.defaultMode) {
                if semTimedOutOrBlocked.wait(timeout: DispatchTime.now()) == 0 {
                    timedOutSem.signal()
                    semTimedOutOrBlocked.signal()
            ...
            let now = DispatchTime.now() + Double(Int64(forcefullyAbortTimeout * Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC)
            let didNotTimeOut = timedOutSem.wait(timeout: now) != 0
            let timeoutWasNotTriggered = semTimedOutOrBlocked.wait(timeout: 0) == 0
    ...
    func wait(_ fnName: String = #function, file: FileString = #file, line: UInt = #line) -> AwaitResult<T> {
        ...
        capture.try {
            ...
            while self.promise.asyncResult.isIncomplete() {
                ...
                RunLoop.current.run(mode: RunLoop Mode.defaultRunLoopMode, before: Date.distantFuture)
            }
...
internal class Awaiter {
    ...
    private func createTimerSource(_ queue: DispatchQueue) -> DispatchSource {
        return DispatchSource.makeTimerSource(flags: DispatchSource.TimerFlags.strict, queue: queue) /*Migrator FIXME: Use DispatchSourceTimer to avoid the cast*/ as! DispatchSource
    }
